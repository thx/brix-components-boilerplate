...
gulp.task('css:concat', ['css:less'], (cb) => {
    var files = []
    gulp.src(['src/**/*.css', '!src/all.css']) // .pipe(debug('css:concat'))
        .pipe(through.obj(
        ...
        .pipe(concat('all.css'))
        .pipe(gulp.dest('./src/')) // .pipe(gulp.dest(DIST + 'styles/'))
        .on('end', cb)
})
...
gulp.task('css:lint', ['css:less', 'css:concat'], (cb) => {
    gulp.src(['src/all.css']).pipe(cache('css:lint')).pipe(debug('css:lint'))
